...
wire_1_bounds = wire_bounds(wire_1)
wire_2_bounds = wire_bounds(wire_2)

x_left = abs(min([wire_1_bounds["x_min"], wire_2_bounds["x_min"]]))
x_right = max([wire_1_bounds["x_max"], wire_2_bounds["x_max"]])
x_length = x_left + x_right + 1

y_down = abs(min([wire_1_bounds["y_min"], wire_2_bounds["y_min"]]))
y_up = max([wire_1_bounds["y_max"], wire_2_bounds["y_max"]])
y_length = y_down + y_up + 1

board = np.zeros((x_length, y_length))

origin = [x_left, y_down]
...
for instruction in wire_2:
    ...
    for i in range(distance):
        ...
        if board[x,y] == 1:
            crosses.append([x,y])
            crosses_with_distance.append([x, y, 0, total_distance_2])

distances = [manhattan_distance(origin, cross) for cross in crosses]
print(min(distances))

# part 2

min_distance = 1000000
for index in range(len(crosses_with_distance)):
    x, y = origin
    total_distance_1 = 0
    for instruction in wire_1:
        direction = instruction[0]
        ...
        for i in range(distance):
            ...
            if direction == "R":
                ...
                y -= 1
            if [x,y] == crosses_with_distance[index][0:2]:
                crosses_with_distance[index][2] = total_distance_1
                break

for coordinates in crosses_with_distance:
    total_distance = coordinates[2] + coordinates[3]
